...
def compile_aithor_exec_file(expt_name):
    ...
    # Replace ANY `robots = [...]` line in the generated code with the real robots list.
    # (LLM sometimes emits robots = [robot1] / [] / ['robot1'] etc. — bare names crash.)
    if robots_line:
        allocated_plan = re.sub(
            r'(?m)^[ \t]*robots\s*=\s*\[[^\n]*$',
            lambda m: robots_line.strip(),
            allocated_plan,
        )
