...
robots = [{'name': 'robot1', 'skills': ['GoToObject', 'BreakObject', 'SwitchOn', 'SwitchOff', 'PickupObject', 'PutObject', 'DropHandObject', 'ThrowObject', 'PushObject', 'PullObject'], 'mass_capacity': 100}, {'name': 'robot2', 'skills': ['GoToObject', 'PickupObject', 'PutObject'], 'mass': 100}, {'name': 'robot3', 'skills': ['GoToObject', 'BreakObject', 'SliceObject', 'SwitchOn', 'SwitchOff', 'PickupObject', 'PutObject', 'DropHandObject', 'ThrowObject', 'PushObject', 'PullObject'], 'mass_capacity': 100}]
floor_no = 1

ground_truth = [{'name': 'CounterTop', 'contains': ['Lettuce'], 'state': 'None'}]
no_trans_gt = 0
max_trans = 10
...
def _norm_name(s):
    """객체명 정규화: 'spatula_location'/'GarbageCan|...'/'spatula' -> 'spatula'."""
    s = str(s).split("|")[0].strip().lower()
    for suf in ("_location", "_loc", "_object", "_obj"):
        if s.endswith(suf):
            s = s[:-len(suf)]
    return s.replace(" ", "").replace("_", "")


def _match_obj(query, obj_id):
    """LLM이 준 이름(query)이 AI2-THOR objectId의 타입과 맞는지 (대소문자/_location 무시)."""
    q = _norm_name(query)
    t = _norm_name(obj_id)
    if not q or not t:
        return False
...
def GoToObject(robots, dest_obj):
    ...
    if "|" in dest_obj:
        ...
        for idx, obj in enumerate(objs):
            if _match_obj(dest_obj, obj):
                dest_obj_id = obj
                dest_obj_center = objs_center[idx]
                if dest_obj_center != {'x': 0.0, 'y': 0.0, 'z': 0.0}:
                    break # find the first instance
    if dest_obj_id is None:
        print(f"[GoToObject] '{dest_obj}' 에 맞는 객체 없음. 가능한 타입:",
              sorted(set(o.split('|')[0] for o in objs)))
        return
...
def PickupObject(robots, pick_obj):
    ...
    for idx in range(no_agents):
        ...
        dest_obj_center = None
        for idx, obj in enumerate(objs):
            if _match_obj(pick_obj, obj):
                pick_obj_id = obj
                dest_obj_center = objs_center[idx]
                if dest_obj_center != {'x': 0.0, 'y': 0.0, 'z': 0.0}:
                    break # find the first instance
        if pick_obj_id is None:
            print(f"[PickupObject] '{pick_obj}' 에 맞는 객체 없음. 건너뜀.")
            continue
...
def PutObject(robot, put_obj, recp):
    ...
    if recp_obj_id is None:
        print(f"[PutObject] '{recp}' 에 맞는 받는 객체 없음. 건너뜀.")
        return
